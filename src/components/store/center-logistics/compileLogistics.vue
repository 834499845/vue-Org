...
export default {
    ...
    if (userRoleOBj) {
      ...
      if (arrs.indexOf("system") == -1) {
        ...
        if (comms.indexOf("components") == -1) {
          ...
          // this.gainstateList()
          this.editLogData = JSON.parse(sessionStorage.logdata);
          //   console.log("修改数据", this.editLogData);
          this.id = this.editLogData.id;
          this.dispatchingModel = this.editLogData.expressDelivery;
          this.tfshow = this.editLogData.enable;
          this.enable = this.editLogData.enable;
          this.firstWeight = this.editLogData.firstWeight;
          this.postage = this.editLogData.postage;
          this.additionalWeight = this.editLogData.additionalWeight;
          this.renew = this.editLogData.renew;
          this.area = this.editLogData.area;
          var arr = this.editLogData.area;
          var arrs = arr.split(","); //通过逗号把字符串分隔
          //   console.log("修改的省市区", arrs);
          if (arrs.length == 1) {
            var self = this;
            self
              .getstateList({
              ...
              .then(data => {
                // console.log("国家s", data);
                if (data.code == 200) {
                  if (data.data) {
                    if (data.data.length) {
                      this.state = data.data;
                      this.statemodel = arrs[0];
                      this.stateCha();
                    } else {
                      this.state = [];
                    }
                  ...
                  if (data.data) {
                    if (data.data.length > 0) {
                      ...
                      self
                        ...
                        .then(data => {
                          //   console.log("省区s", data);
                          if (data.code == 200) {
                            if (data.data) {
                              if (data.data.length > 0) {
                                var objState = data.data;
                                var aarr = [{ value: "请选择", key: "QZX" }];
                                objState.forEach(function(ele, ind) {
                                  aarr.push({ value: ele.value, key: ele.key });
                                });
                                self.province = aarr;
                                self.provincemodel = arrs[1];
                                self.provinceCha();
                              } else {
                                self.$message({
                                  type: "info",
                    ...
                    .then(data => {
                      ...
                      if (data.code == 200) {
                        if (data.data) {
                          if (data.data.length > 0) {
                            var objState = data.data;
                            var arrA = [{ value: "请选择", key: "QZX" }];
                            objState.forEach(function(ele, ind) {
                              arrA.push({ value: ele.value, key: ele.key });
                            });
                            self.province = arrA;
                            self.provincemodel = arrs[1];
                            self
                              ...
                              .then(data => {
                                // console.log("市区s", data);
                                if (data.code == 200) {
                                  if (data.data) {
                                    if (data.data.length > 0) {
                                      var objState = data.data;
                                      var arrB = [{ value: "请选择", key: "QZX" }];
                                      objState.forEach(function(ele, ind) {
                                        arrB.push({
                                          value: ele.value,
                                          key: ele.key
            ...
            // console.log(arrs[0], arrs[1], arrs[2], arrs[3]);
            var self = this;
            self
              .getstateList({
                code: "COUNTRY",
  ...
  methods: {
    ...
    ...mapActions(["getstateList", "getShowlogistics"])
  },
  watch: {
    //配送方式
    ...
    firstWeight: function(val) {
      this.inpRed = false;
      var a = (this.firstWeight).toString();
      var b = a.replace(/[^\d.]/g, "");
      this.firstWeight = b;
    },
    //首重元
    ...
    additionalWeight: function(val) {
      this.inpRed = false;
      var a = (this.additionalWeight).toString();
      var b = a.replace(/[^\d.]/g, "");
