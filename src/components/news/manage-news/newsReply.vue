    ...
    export default {
        data() {
            return {
                ...
                idstring: '', //操作的id
                state: '', //状态：0：是 1：否
                isShowUp: true, //显示接口判断
                typeReply: "",   //评论类型：0：新闻、1：商城、2：医生、3：圈子、4：视频、5：新闻-动态
                currentPage: 1,
                pageSizeNum: 10,
                totalNum: 6,
            };
        ...
        created: function () {
            ...
            if (userRoleOBj) {
                ...
                if (arrs.indexOf("newsList") == -1 && arrs.indexOf("video") == -1 && arrs.indexOf("user") == -1 && arrs.indexOf("audit") == -1) {
                    this.intercept = true;
                    ...
                    if (this.$route.query.newsShow == 'newsNumber') {
                        this.titleNameVal = '新闻评论回复列表'
                        this.typeReply = 5
                    } else if (this.$route.query.newsShow == 'userNumber') {
                        this.titleNameVal = '用户文章评论回复列表'
                        this.typeReply = 5
                    } else if (this.$route.query.newsShow == 'videoNumber') {
                        this.typeReply = 4
                        if (this.$route.query.videoNumber == 'video') {
                            this.titleNameVal = '小视频评论回复列表'
                        } else if (this.$route.query.videoNumber == 'auditContent') {
                            this.titleNameVal = '审核视频评论回复列表'
                        }
                    } else if (this.$route.query.newsShow == 'auditContent') {
                        this.typeReply = 5
                        this.titleNameVal = '审核动态回复列表'
                    }
        ...
        methods: {
            ...
            getnewsReplyList() {
                ...
                self.getcommentdetailList({
                    ...
                    type: self.typeReply,//评论类型：0：新闻、1：商城、2：医生、3：圈子、4：视频、5：新闻-动态
                }).then(data => {
                    self.newsNumList = []
                    if (data.code == 200) {
