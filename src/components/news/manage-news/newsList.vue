<template xmlns:v-on="http://www.w3.org/1999/xhtml">
    <div class="dynamic">
        <div>
            ...
            <div class="search">
                <div>
                    <select v-model="selOneVal" @change="selOneValChange">
                        <option v-for="(type,index) in selOneList" :value="type.id" :id="type.id">{{type.name}}</option>
                    </select>
                </div>
                <div>
                    <select v-model="selTwoVal" @change="selTwoValChange">
                        <option v-for="(type,index) in selTwoList" :value="type.id" :id="type.id">{{type.name}}</option>
                    </select>
                </div>
                <div>
                    <select v-model="selThreeVal" @change="selThreeValChange">
                        <option v-for="(type,index) in selThreeList" :value="type.id" :id="type.id">{{type.name}}</option>
                    </select>
                </div>
                <div class="searchBox">
                    <img src="/static/img/search_1.png" alt="">
                    <input type="text" placeholder="按文章标题搜索" v-model.trim="headlineVal">
                </div>
                <div class="searchBox">
                    <img src="/static/img/search_1.png" alt="">
                ...
                <div class="selectTime">
                    ...
                    <div>
                        <el-date-picker :editable='false' :clearable='true' @change="overtimeChange" v-model="finishTimeVal" type="date" placeholder="请选择日期">
                        </el-date-picker>
                    </div>
                </div>
                <div class="searBut">
                    <div @click="okScreenClick">
                        确定
                    </div>
            ...
            <div class="list">
                <ul class="listContent">
                    ...
                    <li class="dataLi" v-for="(item,index) in dataList">
                        ...
                        <el-tooltip :disabled="item.title && item.title.length > 8 ? false : true" popper-class="toolexam" id="toolselcont" :content="item.title"
                            placement="bottom" effect="light">
                            <div>
                                <span class="greenFont" @click="titleClick(index)">
                                    {{item.title}}
                                </span>
                            </div>
                        </el-tooltip>
                        ...
                        <div>
                            <img @click="uncommonClick(index)" v-show="item.recommend == 1" src="/static/img/shi.png" alt="推荐" title="推荐">
                            <img @click="uncommonClick(index)" v-show="item.recommend == 2" src="/static/img/fou.png" alt="关闭" title="关闭">
                        </div>
                        <div class="listBut">
                            <div>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                beginTimeVal: "", //开始日期标准时间
                finishTimeVal: "", //结束日期标准时间
                timeNumOne: "",//开始时间戳
                timeNumTwo: "",//结束时间戳
                timeOne: "",  //开始年-月-日
                timeTwo: "",  //结束年-月-日
                ...
                dataList: [],   //列表数组
                selOneVal: '', //第一个下拉
                selOneList: [{ id: '', name: '请选择分类' }], //第一个下拉数组
                selTwoVal: '2',//第二个下拉
                selTwoList: [
                    { name: '请选择互动条件', id: '2' },
                    { name: '按互动数最多', id: '0' },
                ...
                idarr: [], //批量操作的id数组
                stateVal: '', //判断推荐还是关闭推荐
                isUncommonUp: true, //推荐请求判断
                isListUp: true, //列表请求判断
                isdeleteUp: true, //列表请求判断
            };
        },
        // 过滤
        filters: {
        ...
        // 模块新建时
        created: function () {
            if (sessionStorage.getItem('newsListSelect')) {
                this.okSearShow = 1
                var objSelect = JSON.parse(sessionStorage.getItem('newsListSelect'))
                this.selOneVal = objSelect.selOneVal//分类id
                this.selThreeVal = objSelect.selThreeVal  //操作选择(0推荐,1不推荐)
                this.selTwoVal = objSelect.selTwoVal  //排序方式(0互动最多排序,1互动最少排序,2按时间排序)
                this.headlineVal = objSelect.headlineVal//标题
                this.authorVal = objSelect.authorVal  //作者
            ...
            if (sessionStorage.getItem('newsListPage')) {
                var objPage = JSON.parse(sessionStorage.getItem('newsListPage'))
        ...
        methods: {
            ...
            getnwesList() {
                ...
                self.getnewsControllerSelectNewsList({
                    type: 0,    //0-新闻，1-动态
                    classifyId: self.selOneVal, //分类id
                        ...
                        if (data.data) {
                            if (data.data.length > 0) {
                                data.data.forEach(function (ele, ind) {
                                    self.selOneList.push({
                                        id: ele.id, name: ele.name
                                    })
                                })
                                console.log(self.selOneList)
                                if (sessionStorage.getItem('newsListSelect')) {
                                    self.selOneVal = JSON.parse(sessionStorage.getItem('newsListSelect')).selOneVal
                                }
                            }
                        }
                    } else {
            ...
            okScreenClick() {
                ...
                if (this.timeShow == 0 && this.selOneVal == '' && this.selTwoVal == '2' && this.selThreeVal == '' && this.headlineVal == '' && this.authorVal == '') {
                    if (this.timeShow == 0) {
                        this.beginTimeVal = this.timeOne = ''
                        this.finishTimeVal = this.timeTwo = ''
                    }
            ...
            getdeleteNews() {
                if (this.isdeleteUp) {
                    ...
                    self.getControllerDelete({
                        newsType: 0, //类型（0：新闻，1：动态）
                        newsIds: self.idstring
                    }).then(data => {
                        self.isdeleteUp = true
                        if (data.code == 200) {
            ...
            addNewsClick() {
                this.$router.push({
                    path: '/news/addNews',
                    query: { newsShow: 'newsList' }
                })
                if (this.okSearShow) {
                    var objOne = {
                        selOneVal: this.selOneVal,//分类id
                    ...
                    sessionStorage.setItem('newsListSelect', strOne)
                }
                ...
                sessionStorage.setItem('newsListPage', strTwo)
            },
            //编辑新闻
            updateClick(index) {
                this.$router.push({
            ...
            numberClick(index) {
                if (this.dataList[index].totalCount > 0) {
                    sessionStorage.setItem("newsId", this.dataList[index].id)
                    this.$router.push({
            ...
            ...mapActions(['getnewsControllerSelectNewsList', 'getclassiflyControlClassifyFindAll', 'getnewsControllerRecommendNews', 'getControllerDelete'])
        },
        // 监视
        watch: {
            headlineVal:function(val) {
                if (val.length == 0) {
                    this.getnwesList()
                }
            },
