        ...
        <div>
            ...
            <div class="list">
                ...
                <div class="listLi">
                    ...
                    <div class="author">
                        <select v-model="classifyVal" @change="classifyChange">
                            <option v-for="(type,index) in classifyList" v-bind:value="type.id" :id="type.id">{{type.name}}</option>
                        </select>
                    </div>
                </div>
                <!-- <div class="listLi">
    ...
    export default {
        ...
        created: function () {
            ...
            if (userRoleOBj) {
                ...
                if (arrs.indexOf("newsList") == -1) {
                    ...
                    //获取新闻详情
                    var self = this
                    self.getnewsControllerSelectNewsDetail({
                        type: 0,
        ...
        methods: {
                    ...
                    if (data.code == 200) {
                        if (data.data.records) {
                            if (data.data.records.length > 0) {
                                self.authorList = data.data.records;
                                self.authorVal = self.nwesData.userId
                            }
                        }
                    } else {
                        self.$message(data.message);
                    ...
                    if (data.code == 200) {
                        if (data.data) {
                            if (data.data.length > 0) {
                                self.classifyList = data.data
                                self.classifyVal = self.nwesData.classifyId
                        ...
                        if (data.code == 200) {
                            if (data.data.records) {
                                if (data.data.records.length > 0) { //分类下有兴趣
                                    ...
                                    if (this.excessivelabelList.length == 0) {  //没有已选的兴趣
                                        ...
                                        if (self.screenLabelShow == false) {    //没有进行按条件查询兴趣
                                            if (self.okLabelIdList.length == 0) {   //原新闻没有兴趣或者是编辑新闻才开始选兴趣
                                                self.oklabelList = [{ name: '请选择兴趣', id: 'selectLabel' }]
                                            } else {    //原新闻有兴趣
                                                self.oklabelList = []
                                                for (var i = 0; i < arr.length; i++) {  //循环当前所有兴趣
                                                    var isLabelId = false
                                                    for (var j = 0; j < self.okLabelIdList.length; j++) {   //原新闻兴趣id
                                                        if (arr[i].labelId == self.okLabelIdList[j]) {
                                                            isLabelId = true
                                                        }
                                                    }
                                                    if (isLabelId) {    //将原新闻已有兴趣展示
            ...
            createEditor() {
                ...
                // editor.config.uploadImgUrl = self.uploadUrl
                // 上传图片（举例）
            ...
            publishClick() {
                ...
                if (regOne.test(this.titleVal) || regTwo.test(this.titleVal) || this.videosSrc.length > 1 || (this.editorImg.length == 0 && this.videosSrc.length == 0 && this.inputText.length == 0)) {
                    ...
                    if (upShow) {
                        ...
                        if (self.isUpEditNews) {
                            self.isUpEditNews = false
                            self.getnewsControllerUpdate({
                                newsId: self.$route.query.id,   //新闻id
                                title: self.titleVal,   //标题
                                userId: self.authorVal, //作者
                                classifyId: self.classifyVal,   //分类
                                labelIds: lableIds,     //兴趣
            ...
            saverDraftClick() {
                ...
                if (regOne.test(this.titleVal) || regTwo.test(this.titleVal) || this.videosSrc.length > 1 || ( this.editorImg.length == 0 && this.videosSrc.length == 0 && this.inputText.length == 0 )) {
                    ...
                    if (upShow == 1) {
                        ...
                        var keyWords = this.keywordList.join(" ")
                        if (self.isUpAddDraft) {
                            self.isUpAddDraft = false
            ...
            ...mapActions(['getUploawdFileVideo', 'getlabelControllerlistLabels', 'getnewsAuthorControllerselectAuthorList', 'getclassiflyControlClassifyFindAll', 'getnewsControllerSelectNewsDetail', 'getnewsControllerUpdate', 'getnewsControllerSaveNews'])
        },
        // 监视
        watch: {
            ...
            keywordVal: function (val) {
                var keywordStr = val
                this.keywordVal = keywordStr.replace(/\s/g, "")
                this.isKeyword = 0
            }
        },
    }
