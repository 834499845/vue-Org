    ...
    <div class="author">
            ...
            <div class="list">
                <ul class="listContent">
                    ...
                    <li class="dataLi" v-for="(item,index) in dataList">
                        ...
                        <div class="listBut">
                            <div>
                                <img @click="editAuthorClick(index)" src="/static/img/编辑.png">
                            </div>
                            <div>
                                <img v-if="item.useNumber == 0" @click="recycleClick(index)" src="/static/img/删除.png">
                                <img v-else src="/static/img/删除灰.png">
                            </div>
                        </div>
                    </li>
                </ul>
        ...
        <div class="ModalsBox" v-show="isModalsBox">
            <div class="Modals modalMain">
                <div class="modalTitle">{{authorTitle}}</div>
                <div class="modalContent">
                    <span>作者名称：</span>
                    <input maxlength="5" :class="{redBorder:isredBorder}" type="text" v-model.trim="authorNameVal">
                ...
                <div class="btnBox">
                    <input type="button" value="确定" @click="addAuthorSureClick" v-show="authorTitle == '新增作者'">
                    <input type="button" value="确定" @click="editAuthorSureClick" v-show="authorTitle == '编辑作者'">
    ...
    export default {
        ...
        data() {
            return {
                ...
                titlename: "该作者将被删除，该操作无法恢复", //删除弹框提示语
                delIndex: "", //删除index
                idstring: "", //序号
                idsArr: [], //多选，所有的id集合
                idsStr: "", //所有的id字符串集合
                ...
                totalNum: 10,
                currentPage: 1,
                authorTitle: "新增作者", //新增作者弹框标题
                iscolorRed: false, //新增作者弹框内的input红字体
                isredBorder: false, //新增作者弹框内的input红边框
                authorNameVal: "", //新增作者弹框内的input值
                isModalsBox: false, //新增作者弹框是否出现
                authorVal: "", //搜索的作者名称,
        ...
        methods: {
            // 获取作者列表
            getAuthorListsFun(val) {
                this.checkList = []
                this.checked = false
                var self = this;
                self
                    .getnewsAuthorControllerselectAuthorList({
                        keyword: val,
            ...
            okSearchClick() {
                if (this.authorVal != '') {
                    ...
                    this.getAuthorListsFun(this.authorVal);
                }
            },
            //重置筛选
            resetClick() {
            ...
            batchClick() {
                if (this.checkList.length > 0) {
                    var idArr = this.idsStr.split(',')
                    var numArr = []
                    for (var i = 0; i < idArr.length; i++) {
                        for (var j = 0; j < this.dataList.length; j++) {
                            if (idArr[i] == this.dataList[j].id) {
                                numArr.push(this.dataList[j].useNumber)
                            }
                        }
                    }
                    var isDel = true
                    ...
                    if (isDel) {
                        ...
                        this.$message("你选中了不可删除的作者");
                    }
                } else {
                    this.$message("请选择要删除的作者");
                }
            },
            //删除
            recycleClick(index) {
                if (this.dataList[index].useNumber == 0) {
                    this.idsArr = [];
                    this.idsStr = "";
                    this.isShowdel = true;
                    this.idsStr = this.dataList[index].id;
                }
            },
            //取消删除(取消所有弹框)
            canleClick() {
            ...
            okClick() {
                ...
                this.isShowdel = false;
                self
                    .getnewsAuthorControllerdeleteAuthor({
                        newsAuthorIds: self.idsStr
                    })
                    .then(data => {
                        if (data.code == 200) {
                            self.$message({
                            ...
                            if (self.okSearchShow == true) {
                                self.getAuthorListsFun(self.authorVal);
                            } else {
            ...
            // 点击新增作者按钮
            addAuthorClick() {
                this.authorTitle = "新增作者";
                this.authorNameVal = "";
                this.isModalsBox = true;
                this.iscolorRed = this.isredBorder = false;
            },
            // 点击编辑作者按钮
            editAuthorClick(index) {
                this.authorTitle = "编辑作者";
                this.isModalsBox = true;
                this.authorNameVal = this.dataList[index].name;
                this.delIndex = index;
            },
            // 作者验证规则
            validateFun() {
                ...
                if (reg_1.test(self.authorNameVal)) {
                    self.iscolorRed = self.isredBorder = true;
                    return false;
                }
                if (reg_2.test(self.authorNameVal)) {
                    self.iscolorRed = self.isredBorder = true;
                    return false;
                }
            },
            ...
            addAuthorSureClick() {
                ...
                if (self.validateFun() == false) {
                    ...
                    this.isModalsBox = false;
                    self
                        .getnewsAuthorControlleraddNewsAuthor({
                            name: self.authorNameVal
                        })
                        .then(data => {
                            if (data.code == 200) {
                                self.$message({
            ...
            editAuthorSureClick() {
                ...
                if (self.validateFun() == false) {
                    ...
                    this.isModalsBox = false;
                    self
                        .getnewsAuthorControllerupdateAuthor({
                            name: self.authorNameVal,
                            newsAuthorId: self.dataList[self.delIndex].id
                        })
                        .then(data => {
                            if (data.code == 200) {
                                self.$message({
