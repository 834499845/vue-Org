            ...
            <div class="filtrate">
                <div class="selectOne">
                    <select v-model="auditModel" @change="auditChange">
                        <option v-for="(state,index) in auditState" v-bind:value="state.key" :key='index'>{{state.value}}</option>
                    </select>
                </div>
                <div class="searchBox interval">
                    <img src="/static/img/search_1.png" alt="搜索">
                    <input type="text" placeholder="输入真实姓名和账号" v-model="searchWordVal">
                </div>
                <!-- 日期 -->
                ...
                <div class="listContent">
                    <ul>
                        ...
                        <div class="noList" v-show="dataList.length == 0">
                            <div>
                                <img src="/static/img/w.png" alt="暂无数据">
                            </div>
                        </div>
                        <li class="dataLi" v-for="(item,index) in dataList" :key='index'>
                            ...
                            <div>
                                {{item.accountState | stateFun}}
                            </div>
                            <div>
                                {{item.state | auditFun}}
                            </div>
                            <div>
                               {{item.applyDate | timeFun}}                               
                            </div>
                            <div class="operation">
                                <div>
                                    <img v-if="item.state == 0" src="/static/img/审核.png" @click="checkPendingClick(index)" alt="审核" title="审核">
...
export default {
  ...
  created: function() {
    var self = this;
    if (sessionStorage.getItem("auditCurrentPage")) {
      self.currentPage = Number(sessionStorage.getItem("auditCurrentPage"));
      self.pageSizeNum = Number(sessionStorage.getItem("auditPageSizeNum"));
    }
    // 权限限制
    ...
    if (userRoleOBj) {
      ...
      if (arrs.indexOf("clientSideAccount") == -1) {
        ...
        if (
          sessionStorage.getItem("auditSearchWordVal") ||
          sessionStorage.getItem("auditModelVal") ||
          (sessionStorage.getItem("timeFirstVal") &&
            sessionStorage.getItem("timeSecondVal"))
        ) {
          if (sessionStorage.getItem("auditSearchWordVal")) {
            self.searchWordValUp = self.searchWordVal = sessionStorage.getItem("auditSearchWordVal");
          }
  ...
  methods: {
    ...
    getDataListFun() {
      var self = this;
      sessionStorage.removeItem("auditCurrentPage");
      sessionStorage.removeItem("auditPageSizeNum");
      if (self.filterClickTrueOrFalse == true) {
        self.filterClickTrueOrFalse = false;
        self
          .getClientSideIdentityAuditGetApplyDataListByCondition({
            pageNo: self.currentPage,
            pageSize: self.pageSizeNum,
            beginDate: self.timeOne,
    ...
    searchClick() {
      ...
      sessionStorage.removeItem("auditSearchWordVal");
    ...
    updateClick(index) {
      var self = this;
      sessionStorage.setItem("id", self.dataList[index].id);
      sessionStorage.setItem("auditCurrentPage", self.currentPage);
      sessionStorage.setItem("auditPageSizeNum", self.pageSizeNum);
      sessionStorage.setItem("auditSearchWordVal", self.searchWordVal);
    ...
    ...mapActions(["getClientSideIdentityAuditGetApplyDataListByCondition"])
  },
  watch: {
