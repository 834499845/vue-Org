...
export default {
  data() {
    return {
      // isRotateCl: 0,
      isTweSan: 0,// 点击左侧导航时的样式控制
      isOneSan: 0,// 选中左侧导航时背景颜色控制
      isliOne: true,// 控制左侧导航样式
      lists: [],// 左侧导航列表
      // twoShow: 0,
      userName: "暂无名称"// 名称
    };
          ...
          if (userRoleOBj) {
            ...
            if (self.lists.length > 0) {
              ...
              arrs.forEach(function(ele, ind) {
                ...
                if (oneStr == "clientAudit" || oneStr == "clientLookDetail") {
                  if (ele == "clientDataAudit") {
                    // self.twoShow = ind;
                    // self.isRotateCl = ind;
                    self.isTweSan = ind;
                    self.isOneSan = ind;
                  ...
                  oneStr == "clientApprove" ||
                  oneStr == "clientSeeDetail"
                ) {
                  if (ele == "clientAccountAppeal") {
                    // self.twoShow = ind;
                    // self.isRotateCl = ind;
                    self.isTweSan = ind;
                    self.isOneSan = ind;
