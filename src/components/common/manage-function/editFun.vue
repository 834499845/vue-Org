        ...
        <div>
            ...
            <div class="list">
                <div class="list-code">
                    ...
                    <div>
                        <input type="text" v-model="titleVal" :class="{redInput:isRedtitleVal,grayInput:isGraytitleVal}">
                    </div>
                ...
                <div class="list-link">
                    ...
                    <div class="list-url">
                        <select @change="selectcodesClick" v-model="codestr" :class="{redInput:isRedbestirLinkUrl,grayInput:isGraybestirLinkUrl}">
                            <option v-for="(type,index) in codes" :key="index" v-bind:value="type.dictionaryCode">{{type.dictionaryName}}</option>
                        </select>
                        <div>
                          {{bestirLinkUrl}}
                        </div>
                    </div>
                    <div class="list-tost-url" :class="{redTest:isRedbestirLinkUrl,grayTest:isGraybestirLinkUrl}">
                      活动链接必填
                    </div>
                </div>
                <div class="list-code">
                    ...
                    <div>
                        <Date-picker :clearable="true" :editable="false" v-model="startDateVal" type="datetime" format="yyyy-MM-dd HH:mm"
                                placeholder="选择开始时间" style="width: 200px"></Date-picker>
                    </div>
                    <div class="list-tost" :class="{redTest:isRedstartDateVal,grayTest:isGraystartDateVal}">
                      开始时间必填,不能小于当前时间且不能大于结束时间
                    </div>
                </div>
                <div class="list-code">
                    <div>
                    ...
                    <div>
                        <Date-picker :clearable="true" :editable="false" v-model="endDateVal" type="datetime" format="yyyy-MM-dd HH:mm"
                                placeholder="选择结束时间" style="width: 200px"></Date-picker>
                    </div>
                    <div class="list-tost" :class="{redTest:isRedendDateVal,grayTest:isGrayendDateVal}">
                      结束时间必填,且不能小于开始时间
                    </div>
                </div>
                <div class="list-code">
                    ...
                    <div>
                        <input type="text" v-model="launchVal" :class="{redInput:isRedlaunchVal,grayInput:isGraylaunchVal}">
                    </div>
...
export default {
  ...
  data() {
    return {
      ...
      codestr: "QINGXUNZE",
      bestirLinkUrl: "",
      isRedbestirLinkUrl: false,
      isGraybestirLinkUrl: true,
      isbestirLinkUrl: false,
    ...
    if (userRoleOBj) {
      ...
      if (arrs.indexOf("fun") == -1) {
        ...
        self
          ...
          .then(data => {
            if (data.code == 200) {
              self.bestirLinkUrl = data.data.url;
              self.contentVal = data.data.content;
              self.startDateVal = data.data.startDate;
                  ...
                  if (data.code == 200) {
                    if (data.data) {
                      if (data.data.length > 0) {
                        ...
                        var bestirLinkUrlStr = self.bestirLinkUrl.split(
                          "code="
                        )[1];
                        self.codes.forEach(function(ele, i) {
                          if (ele.dictionaryCode == bestirLinkUrlStr) {
                            self.codestr = ele.dictionaryCode;
                          }
                        });
                        if (self.codestr == "QINGXUNZE") {
                          self.isbestirLinkUrl = false;
                        } else {
                          self.isbestirLinkUrl = true;
                          this.bestirLinkUrl =
                            global.httpStr +
                            global.banner +
                            "/#/encourage?sort=5&code=" +
                            this.codestr;
                          this.urlVal = this.bestirLinkUrl;
                        }
                      } else {
                        self.codes = [];
                      }
  ...
  methods: {
    // 选择内容下拉
    selectcodesClick() {
      if (this.codestr == "QINGXUNZE") {
        this.isbestirLinkUrl = false;
      } else {
        this.isbestirLinkUrl = true;
        this.bestirLinkUrl =
          global.httpStr +
          global.banner +
          "/#/encourage?sort=5&code=" +
    ...
    addDoct() {
      var self = this;
      if (self.endDateVal) {
        var endTime = new Date(self.endDateVal).getTime();
        var startTime = new Date(self.startDateVal).getTime();
        if (startTime * 1 > endTime * 1) {
          self.isendDateVal = false;
        } else {
          self.isendDateVal = true;
        }
      } else {
        self.isendDateVal = false;
      }
      if (self.startDateVal) {
        ...
        if (nowTime * 1 < startTime * 1) {
          if (startTime * 1 > endTime * 1) {
            self.isstartDateVal = false;
          } else {
            self.isstartDateVal = true;
          }
        } else {
          self.isstartDateVal = false;
        }
      ...
      if (
        ...
        if (self.isstartDateVal == false) {
          self.isRedstartDateVal = true;
          self.isGraystartDateVal = false;
        } else {
          self.isRedstartDateVal = false;
          self.isGraystartDateVal = true;
        }
        if (self.isendDateVal == false) {
          self.isRedendDateVal = true;
          self.isGrayendDateVal = false;
        } else {
          self.isRedendDateVal = false;
          self.isGrayendDateVal = true;
        }
        ...
        if (self.isbestirLinkUrl == false) {
          self.isRedbestirLinkUrl = true;
          self.isGraybestirLinkUrl = false;
        } else {
          self.isRedbestirLinkUrl = false;
          self.isGraybestirLinkUrl = true;
        }
      }
    },
    ...mapActions([
