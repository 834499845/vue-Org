        ...
        <div>
            ...
            <div class="list">
                <div>
                    ...
                    <div class="cssFlex">
                        <input :class="{redbor:isborname}" class="listInp redBorder" type="text" maxlength="30" v-model="name">
                        <span :class="{redfont:isfontname}" class="isAstrict">最多输入30个汉字（不支持非法字符）</span>
                    </div>
                </div>
                <div class="upTwoIm">
                    <div>
                ...
                <div class="listlabel">
                    ...
                    <div class="cssFlex">
                        <input :class="{redbor:isborlabel}" class="listLabelInp redBorder" type="text" maxlength="5" v-model="labelName" @keyup.13="show()">
                        <span :class="{redfont:isfontlabel}" class="isAstrict">最多输入5个字符（输入完成请按回车确认、最多添加4个标签、不支持非法字符）</span>
                    </div>
                </div>
                <div class="listLabelSpa">
                    <span v-for="(city,index) in labelArr">
                        {{city}}
                ...
                <div class="listintroduce">
                    ...
                    <div>
                        <textarea :class="{redbor:isborintroduce}" class="listIntroduceInp" maxlength="1500" v-model="introduce"></textarea>
                    </div>
                </div>
                <div class="listintroduceFont">
                    <div></div>
                    <div>
                        <span :class="{redfont:isfontintroduce}" class="isAstrict">最多输入1500个字符，可以输入中文，英文，数字（不支持非法字符）</span>
                    </div>
                </div>
                <div class="list-limit">
                    <div>经度：</div>
                    <div>
                        <input :class="{redbor:isborlong}" type="text" maxlength="11" v-model="longitudeText">
                ...
                <div class="list-limit">
                    <div>纬度：</div>
                    <div>
                        <input :class="{redbor:isborlat}" type="text" maxlength="10" v-model="latitudeText">
                    ...
                    <div>
                        <div>
                            <select :class="{redbor:isborprovince}" @change="selecBradnsClick" v-model="gainProvinceId">
                                <option v-for="(type,index) in provinceList" v-bind:value="type.key">{{type.value}}</option>
                            </select>
                        </div>
                        <div class="districtSmall">
                            <select :class="{redbor:isborcity}" v-model="gainCityId" @change="gainCityChg">
                                <option v-for="(type,index) in cityList" v-bind:value="type.key">{{type.value}}</option>
                            </select>
                        </div>
                    </div>
                </div>
        ...
        created: function () {
            ...
            if (userRoleOBj) {
                ...
                if (arrs.indexOf("hospital") == -1) {
                    ...
                    self.hospitalId = self.editData.id;
                    self.imgboxOne = false;
                    self.imgboxTow = true;
                    self.getHospitalDetail({
                        hospitalId: self.hospitalId
                    }).then(data => {
                        // console.log("医院详情", data);
                        if (data.code == 200) {
                            var hospitaldetails = data.data;
                            var self = this;
                            self.mainPicuter = hospitaldetails.itemImage;
                            self.name = hospitaldetails.hospitalName;
                            self.introduce = hospitaldetails.instruction;
                            self.longitudeText = hospitaldetails.longitude;
                            self.latitudeText = hospitaldetails.latitude;
                            self.hospitalsite = hospitaldetails.address;
                            //标签
                            var labelObj = "";
                            if (hospitaldetails.label.indexOf(",") >= 0) {
                                var labelObj = hospitaldetails.label.split(",");
                                for (var i = 0; i < labelObj.length; i++) {
                                    self.labelArr.push(labelObj[i]);
                                }
                            } else {
                                self.labelArr.push(hospitaldetails.label);
                            }
                            //级别
                            self.adminArry = hospitaldetails.rankDivide;
                            self.jbList();
                            //省市
                            self.getprovince({
                                code: "PROVINCE",
                                ...
                                if (data.code == 200) {
                                    if (data.data) {
                                        if (data.data.length > 0) {
                                            var asd = [];
                                            asd = data.data;
                                            self.provinceList = asd;
                                            self.gainProvinceId = hospitaldetails.provinceId;
                                            //*****获取市级列表*************************************** */
                                            if (self.gainProvinceId) {
                                                self.getprovince({
                                                    code: "CITY",
                                                    parentId: self.gainProvinceId
                                                }).then(data => {
                                                    // console.log("市级列表", data);
                                                    if (data.code == 200) {
                                                        if (data.data) {
                                                            if (data.data.length > 0) {
                                                                ...
                                                                self.cityList = arr;
                                                                self.gainCityId = hospitaldetails.cityId;
                                                            } else {
                                                                self.cityList = []
                                                            }
                                                        } else {
                            ...
                            // 科室
                            self.department = hospitaldetails.hospitalOfficeDTOList;
                            //一级科室
                            var self = this;
                            self.gettrainedList({
                                ...
                                if (data.code == 200) {
                                    if (data.data) {
                                        if (data.data.length > 0) {
                                            ...
                                            self.bigArray = hospitaldetails.bigOffices;
                                            for (var k = 0; k < self.citiesa.length; k++) {
                                                var ifbig = true;
                                                for (var i = 0; i < self.bigArray.length; i++) {
                                                    if (self.bigArray[i] == self.citiesa[k].id) {
                                                        ifbig = false;
                                                        self.cities.push({
                                                            key: self.citiesa[k].id,
                                                            value: self.citiesa[k].value,
                                                            bigshow: false
                                                        });
                                                    }
                                                }
                                            ...
                                            //*** 二级科室 ************************************************ */
                                            self.smallArray = hospitaldetails.smallOffices;
                                            //**************************************************************** */
                                        } else {
                                            self.citiesa = []
                                        }
        ...
        methods: {
                    ...
                    .then(data => {
                        if (data.code == 200) {
                            if (self.cities[self.bigIndex].bigshow == true) {
                                ...
                                self.citiesaLit = cidsadsd;
                                var arr1 = self.smallArray;
                    ...
                    .then(data => {
                        ...
                        if (data.code == 200) {
                            if (data.data) {
                                if (data.data.length > 0) {
                                    self.levs = data.data;
                                    for (var j = 0; j < self.levs.length; j++) {
                                        if (self.adminArry == self.levs[j].key) {
                                            self.tfshow = j;
                                            self.checkedf = false;
                                            self.checkedt = true;
                                        }
            ...
            uploadImage(file) {
                ...
                if (this.uploadImageTrueOrFalse == true) {
                    ...
                    this.gethosuploadFile(formdata).then(data => {
                        // console.log(data);
