...
<script>
    ...
    export default {
        ...
        // 模块新建时
        created: function () {
            this.editData = JSON.parse(sessionStorage.eDocData)
            var self = this;
            // 权限限制
                    ...
                    self.getselectDoctorById({
                        ...
                        if (data.code == 200) {
                            if (data.data) {
                                ...
                                self.docName = data.data.doctorName
                                if (data.data.itemImage) {
                                    self.mainPicuter = data.data.itemImage
                                    self.imgboxOne = false
                                    self.imgboxTow = true
                                }
                                if (data.data.label) {
                                    self.labelArr = data.data.label.split(",")
                                }
                                if (data.data.instruction) {
                                    self.introduce = data.data.instruction
                                }
                                if (data.data.fieldInstruction) {
                                    self.territory = data.data.fieldInstruction
                                }
                                if (data.data.medicalInstruction) {
                                    self.backdrop = data.data.medicalInstruction
                                }
                                if (data.data.academicInstruction) {
                                    self.achieve = data.data.academicInstruction
                                }
                                if (data.data.doctorWrote) {
                                    self.bailment = data.data.doctorWrote
                                }
                                self.hospitalList()
                                self.gainDoctorRankList()
                            } else {
                                self.$message({
                                    type: 'info',
                                    message: '获取医生详情失败！'
        ...
        methods: {
                        ...
                        if (data.data) {
                            if (data.data.records) {
                                if (data.data.records.length == 0) {
                                    ...
                                    if (this.hospClick == false) {
                                        self.hospId = this.doctorData.hospitalId
                                        self.hospName = this.doctorData.hospitalName
                                    } else {
            ...
            ksList() {
                ...
                this.smallAdminId = ""
                var self = this
                self.getselectBigDepartmentList({
                    pageNo: 1,
                    pageSize: 100000000,
                        ...
                        if (data.data) {
                            if (data.data.records) {
                                if (data.data.records.length > 0) {
                                    ...
                                    if (this.hospClick == true) {
                                        self.BigAdminId = self.bigAdminList[0].id
                                    } else {
                                        self.BigAdminId = self.doctorData.firstDepartmentId
                                    }
                                    self.ksLitList()
                                } else {
            ...
            ksLitList() {
                ...
                this.smallAdminId = ""
                var self = this
                self.getselectSmallDepartmentList({
                    pageNo: 1,
                    pageSize: 1000000000,
                    hospitalId: self.hospId,
                            ...
                            if (data.data.records) {
                                if (data.data.records.length) {
                                    ...
                                    if (this.hospClick == true) {
                                        ...
                                        self.smallAdminId = self.doctorData.secondDepartmentId
                                        self.smallAdminName = self.doctorData.department
                                    }
                                } else {
                                    self.smallAdminList = [{ value: "暂无数据", id: '' }]
                                    self.smallAdminId = ""
                    ...
                    if (data.code == 200) {
                        if (data.data) {
                            if (data.data.length) {
                                ...
                                self.DoctorRank = self.doctorData.doctorLevelId
                                self.DoctorRankName = self.doctorData.doctorLevel
                            } else {
            ...
            uploadImage(file) {
                ...
                if (this.uploadImageTrue == true) {
                    this.uploadImageTrue = false
                    this.gethosuploadFile(formdata).then((data) => {
                    //console.log(data)
                    this.uploadImageTrue = true
                    if (data.code == 200) {
            ...
            verification() {
                ...
                var JudgeTwo = new RegExp("[`！。，.,!-%?？《》：:+`~@#$^&*()=|{}\\[\\]<>/@#￥&*;（）——|{}【】]")
                if (JudgeOne.test(this.docName) || JudgeTwo.test(this.docName) || this.imgboxTow == false || this.hospId == "" || this.BigAdminId == "" || this.smallAdminId == "" || this.DoctorRank == "" || this.labelArr.length == 0 || JudgeOne.test(this.introduce) || JudgeOne.test(this.territory) || JudgeOne.test(this.backdrop) || JudgeOne.test(this.achieve) || JudgeOne.test(this.bailment)) {
                    return false
                }
            },
            //确定编辑医生
            addDoct() {
                if (this.verification() == false) {
                    ...
                    if (self.adddoct == true) {
                        self.adddoct = false
                        self.getEditDoct({
                            doctorId: self.editData.id,     //医生id
            ...
            ...mapActions(['getselectDoctorById', 'getEditDoct', 'gethosuploadFile', 'getselectBigDepartmentList', 'getDoctorRank', 'getselectSmallDepartmentList', 'gethospitalmanageList'])
        },
        // 监视
        watch: {
            //标签
