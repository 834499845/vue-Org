          ...
          <div class="listCont" v-loading="loading">
              <ul>
                  ...
                  <li class="dataLi" v-for="(item,index) in lists" :key="index">
                      <div>{{item.name}}</div>
                      <div>
                        <div v-show="lists[index].name!=='每日签到'">{{item.score}}</div>
                        <div v-show="lists[index].name ==='每日签到'">动态积分</div>
                      </div>
                      <!-- <div >{{item.score}}</div> 
                      <div ></div>   -->
      ...
      <div :class="{popup:isPopup,showPopup:isShowPopup}">
            <transition name="slide-fade">
                ...
                <div class="popupData">
                    ...
                    <div v-show="nameVal !== '每日签到'">
                        <input type="text" v-model="scoreVal" :class="{redBorder:isRedScoreVal,grayBorder:isGrayScoreVal}">
                    </div>
                    <div class="special" v-show="nameVal === '每日签到'">动态积分</div>
                    <div v-show="nameVal !== '每日签到'" :class="{redTest:isRedScoreVal,grayTest:isGrayScoreVal}">积分数量只能为正整数</div>
                </div>
                <div class="popupData">
                    <div>
                        每日上限：
                    ...
                    <div class="special" v-show="nameVal == '每日签到'">{{dayLimitCountVal}}</div>
                    <div v-show="nameVal != '每日签到'" :class="{redTest:isRedDayLimitCountVal,grayTest:isGrayDayLimitCountVal}">请选择每日上限</div>
                </div>
                <div class="popupData">
                    <div>
                        任务有效期：
  ...
  methods: {
    ...
    getFixationListFun() {
      ...
      self
        ...
        .then(data => {
          if (data.code == 200) {
            self.dataLists = data.data[0];
            self.lists = data.data[0].attrList;
    ...
    butUpdateClick() {
      ...
      if (self.validateFun() == false) {
        ...
        if (self.nameVal === "每日签到") {
          var a = 100
          obj.score = Number(a)
          console.log("1")
        } else if (self.nameVal !== "每日签到") {
          obj.score = self.scoreVal
        }
