...
export default {
  ...
  created: function() {
    ...
    var oneStr = paths[2];
    var twoStr = paths[3];
    if (sessionStorage.getItem("recreationIndex")) {
    } else {
      sessionStorage.setItem("recreationIndex", 0);
    }
    self.fetchData();
    if (idStr) {
    } else {
  ...
  methods: {
      ...
      if (idStr) {
        ...
        if (userRoleOBj) {
          ...
          arrs.forEach(function(ele, ind) {
            ...
            if (oneStr == "addFood" || oneStr == "compileFood") {
              if (ele == "food") {
                self.twoShow = ind;
                self.isRotateCl = ind;
              }
            } else if (oneStr == "addRecipe" || oneStr == "compileRecipe") {
              if (ele == "recipe") {
                self.twoShow = ind;
                self.isRotateCl = ind;
              }
            } else if (
              oneStr == "addAssociatedSymptoms" ||
              oneStr == "compileAssociatedSymptoms"
            ) {
              if (ele == "associatedSymptoms") {
                self.twoShow = ind;
                self.isRotateCl = ind;
              }
            }
    ...
    loOneClick(ind) {
      ...
      this.$router.push({
        path: "/recreation/" + pathStr
      });
      sessionStorage.setItem("recreationIndex", ind);
    },
    // 退出登录
    exitClick() {
      sessionStorage.removeItem("recreationIndex");
      this.$router.push({
        path: "/home"
      });
    },
