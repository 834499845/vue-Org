...
<template xmlns:v-on="http://www.w3.org/1999/xhtml">
  <div id="app" class="sale">
    ...
    <div class="foodList">
      <div class="foodName">
        名称：
        <input type="text" maxlength="12" @blur='loseInput' @focus='foodNameInput' v-model='foodName' placeholder="请输入食物名称" />
        <span class="markedWords" v-show='greyPrompt'>最多输入12个字符</span>
        <span class="redMarkedWords" v-show='redPrompt'>最多输入12个字符</span>
      </div>
      <div class="foodPic">
        <div class="picBox">
          <input id="upfile" type="file" value="+" @change="onFileChange" />
          <img v-show='thumbnail' :src="this.zoomPath" />
        </div>
        <div class="uploading">上传图片</div>
        <span class="picGreyPrompt" v-show='picGreyPrompt'>图片文件大小不能超过1M，尺寸为1280*778</span>
        <span class="picMarkedWords" v-show='picMarkedWords'>图片文件大小不能超过1M，尺寸为1280*778</span>
      </div>
      <div class="weightStandard">重量标准：
        <!-- <v-select class='select1' width="178" :selects="selectfl" :list="brands" @select="selecBradnsClick"></v-select> -->
        ...
        <span class="markedWordswei" v-show='greyPromptwei'>请选择重量标准</span>
      </div>
      <div class="foodSpecifications">食物规格：
        <select v-model="foodSpecifications" :class="{foodSpecificationsa:IsfoodSpecifications}" @focus='foodSpecificationsInput'>
          <option value="">请选择</option>
          <option>克</option>
          <option>毫升</option>
        </select>
        <!-- <input type="text" v-model='foodSpecifications' :class="{foodSpecificationsa:IsfoodSpecifications}" @focus='foodSpecificationsInput'/><span class="markedWords">例：克，毫升</span> -->
      </div>
          ...
          <ul>
            <li v-for="(item,index) in unitLists" :key="index">
              <div class="unitListsLeft" @click="checkedOneChangeClick(index)">
                <img v-show="item.unitShow == true" :src="item.unImg" alt="">
                <img v-show="item.unitShow == false" :src="item.img" alt="">
              </div>
      ...
      <div class="handle">
        <div class="foodSave" @click='addfoodSaves'>保存</div>
...
export default {
  ...
  data() {
    return {
      ...
      intercept: false,
      isdelPopup: true,
      selectShowOne: 0, //控制选择下拉框是否出现
      selectShowTwo: false, //控制选择下拉框是否出现
      selectShowThree: false, //控制选择下拉框是否出现
      selectShowFour: false, //控制选择下拉框是否出现
  ...
  methods: {
                ...
                if (data.data.length > 0) {
                  ...
                  data.data.forEach(function(ele, ind) {
                    untilArr.push({
                      ...
                      unitValue: "",
                      unitArr: self.units,
                      unImg: "/static/img/unchecked.png",
    ...
    addfoodSaves() {
      ...
      if (!this.validation()) {
        ...
        console.log(measurementUnitArr);
        if (measurementUnitArr.length > 0) {
          this.isfoodUnitTost = false;
          console.log(measurementUnitArr);
          self
            .getfoodFoodAdd({
              name: self.foodName,
    ...
    validation() {
      ...
      if (chineseCharacters.test(this.foodName) == true) {
        if (this.redPrompt == false) {
        } else {
          this.redPrompt = true;
  ...
  watch: {
    foodName(val) {
      var self = this;
      var chineseCharacters = /[\u4e00-\u9fa5_a-zA-Z0-9_]/;
      if (chineseCharacters.test(val) == true) {
        self
          .getfoodFuzzyQuery({
            name: val
          })
