...
export default {
  ...
  created: function() {
    ...
    if (sessionStorage.getItem("fitnessIndex")) {
    } else {
      sessionStorage.setItem("fitnessIndex", 0);
    }
    if (idStr) {
      self.fetchData();
    } else {
  ...
  methods: {
        ...
        if (userRoleOBj) {
          ...
          if (self.lists.length > 0) {
            if (oneStr == undefined || oneStr == "") {
              self.$router.push({
                path: "/fitness/" + self.lists[0].path
              });
            } else {
            ...
            arrs.forEach(function(ele, ind) {
              ...
              if (oneStr == "lookMovement" || oneStr == "opMovement" ) {
                if (ele == "movement") {
                  self.twoShow = ind;
                  self.isRotateCl = ind;
                  self.twoShow = sessionStorage.getItem("fitnessIndex");
                  self.isRotateCl = sessionStorage.getItem("fitnessIndex")
                }
              } else if (oneStr == "lookDrill" || oneStr == "opDrill" || oneStr == "updateDrill") {
                if (ele == "drill") {
                  self.twoShow = ind;
                  self.isRotateCl = ind;
                  self.twoShow = sessionStorage.getItem("fitnessIndex");
              ...
              } else if (oneStr == "lookCourse" || oneStr == "opCourse" || oneStr == "updateCourse") {
                if (ele == "course") {
                  self.twoShow = ind;
                  self.isRotateCl = ind;
                  self.twoShow = sessionStorage.getItem("fitnessIndex");
    ...
    loOneClick(ind) {
      ...
      this.$router.push({
        path: "/fitness/" + pathStr
      });
      sessionStorage.setItem("fitnessIndex", ind);
    },
    // 退出登录
    exitClick() {
      this.$router.push({
      ...
      sessionStorage.removeItem("fitnessIndex");
    },
