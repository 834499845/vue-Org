            ...
            <div class="menu">
                <div class="menuLeft">
                    <div>
                        ...
                        <div>
                            <input type="text" v-model="searchVal" placeholder="输入名称关键字">
                        </div>
                    </div>
                    <div>
                        <select @change="selectTypesClick" v-model="type">
                            <option v-for="(type,index) in types" :key="index" v-bind:value="index">{{type.labelName}}</option>
                        </select>
                    </div>
                    <div>
                        <select @change="selectPartClick" v-model="part">
                            <option v-for="(type,index) in parts" :key="index"  v-bind:value="index">{{type.siteName}}</option>
                        </select>
                    </div>
                ...
                <div class="listCont" v-loading="loading">
                    <ul>
                        ...
                        <li class="dataLi" v-show="lists.length > 0" v-for="(item,index) in lists">
                            ...
                            <div @click="partNameClick(index)">{{item.courseName | strFun}}</div>
...
export default {
  ...
  created: function() {
    var self = this;
    if (sessionStorage.getItem("coursecurrentPage")) {
      this.currentPage = Number(sessionStorage.getItem("coursecurrentPage"));
      this.pageSizeNum = Number(sessionStorage.getItem("coursepageSizeNum"));
    }
    // 权限限制
    ...
    if (userRoleOBj) {
      ...
      if (arrs.indexOf("course") == -1) {
        ...
        if (
          sessionStorage.getItem("coursesearchVal") ||
          sessionStorage.getItem("courseLabelId") ||
          sessionStorage.getItem("courseSiteId")
        ) {
          if (sessionStorage.getItem("coursesearchVal")) {
            self.searchValUp = self.searchVal = sessionStorage.getItem("coursesearchVal");
            self.searchListFun();
          }
          if (sessionStorage.getItem("courseLabelId")) {
            self.labelNames = sessionStorage.getItem("courseLabelName")
            self.searchListFun();
          }
          if (sessionStorage.getItem("courseSiteId")) {
            self.siteNames = sessionStorage.getItem("courseSiteName")
            self.searchListFun();
          }
        } else {
          self.listFun();
  ...
  methods: {
    ...
    listFun() {
      var self = this;
      self.loading = true;
      sessionStorage.removeItem("coursecurrentPage");
      sessionStorage.removeItem("coursepageSizeNum");
      self
        .getfitnessCourseControllerselectCoursePage({
        ...
        .then(data => {
          self.loading = false;
          self.dataTrue = false;
          //   console.log("课程名称列表", JSON.parse(JSON.stringify(data)));
          if (data.code == 200) {
            if (data.data) {
    ...
    searchListFun() {
      ...
      sessionStorage.removeItem("coursecurrentPage");
      sessionStorage.removeItem("coursepageSizeNum");
      sessionStorage.removeItem("coursesearchVal");
      //带搜索的初始化
      self
        .getfitnessCourseControllerselectCoursePage({
          pageNo: self.currentPage,
          pageSize: self.pageSizeNum,
          courseName: self.searchValUp,
          labelName: self.labelNames,
          siteName: self.siteNames
        })
        .then(data => {
          self.loading = false;
          self.dataTrue = false;
      ...
      self
        ...
        .then(data => {
                ...
                if (data.data.records.length > 0) {
                  ...
                  if (sessionStorage.getItem("courseLabelId")) {
                    self.types.forEach(function(ele, ind) {
                      if (ele.id == sessionStorage.getItem("courseLabelId")) {
                        self.type = ind;
                      }
                    });
                  }
          ...
          sessionStorage.removeItem("courseLabelId");
          sessionStorage.removeItem("courseLabelName");
        })
        .catch(msg => {
          self.lists = [];
          //   console.log(msg);
      ...
      self
        ...
        .then(data => {
                ...
                if (data.data.records.length > 0) {
                  ...
                  if (sessionStorage.getItem("courseSiteId")) {
                    self.parts.forEach(function(ele, ind) {
                      if (ele.id == sessionStorage.getItem("courseSiteId")) {
                        self.part = ind;
          ...
          sessionStorage.removeItem("courseSiteId");
          sessionStorage.removeItem("courseSiteName");
        })
        .catch(msg => {
          self.parts = [{ siteName: "暂无数据" }];
          //   console.log(msg);
    ...
    selectTypesClick() {
      ...
      if (
        ...
        if (self.type != 0) {
          self.labelNames = self.types[self.type].labelName
        }
        self.searchListFun();
      }
    },
    ...
    selectPartClick() {
      ...
      if (
        ...
        if (self.part != 0) {
          self.siteNames = self.parts[self.part].siteName
        }
        self.searchListFun()
      }
    },
    ...
    handleSizeChange(val) {
      ...
      // if (
      //   self.searchVal.length > 0 ||
      //   self.types[self.type].labelName != "请选择" ||
      //   self.parts[self.part].siteName != "请选择"
      // ) {
        self.searchListFun();
      // } else {
      //   self.listFun();
    ...
    updateClick(index) {
      ...
      if (self.searchVal.length > 0) {
        sessionStorage.setItem("coursesearchVal", self.searchVal);
      }
      if (self.types[self.type].labelName != "请选择") {
        sessionStorage.setItem("courseLabelId", self.types[self.type].id);
        sessionStorage.setItem("courseLabelName",self.types[self.type].labelName);
      }
      if (self.parts[self.part].siteName != "请选择") {
        sessionStorage.setItem("courseSiteId", self.parts[self.part].id);
        sessionStorage.setItem("courseSiteName",self.parts[self.part].siteName);
      }
      sessionStorage.setItem("coursecurrentPage", self.currentPage);
      sessionStorage.setItem("coursepageSizeNum", self.pageSizeNum);
      this.$router.push({
    ...
    partNameClick(index) {
      ...
      sessionStorage.setItem("lookCourseId", self.lists[index].id);
    },
    // delete
    deleteClick(index) {
      this.isShowdel = false;
    ...
    delClick() {
      ...
      if (self.delClickTrue == false) {
        ...
        self
          .getfitnessCourseControllerdeleteById({
            ids: idStr
          })
          .then(data => {
            ...
            self.isShowdel = true;
            self.delShow = false;
            // console.log("删除", JSON.parse(JSON.stringify(data)));
            if (data.code == 200) {
              self.$message({
                type: "success",
                message: "删除成功!"
    ...
    searchClick() {
      ...
      if (self.dataTrue == true) {
        ...
        if (
          ...
          // };
          // if (self.searchVal.length > 0) {
          //   searchObj.courseName = self.searchVal;
          // }
          // if (self.types[self.type].labelName != "请选择") {
          //   searchObj.labelName = self.types[self.type].labelName;
          // }
          // if (self.parts[self.part].siteName != "请选择") {
          //   searchObj.siteName = self.parts[self.part].siteName;
          // }
          // self
          //   .getfitnessCourseControllerselectCoursePage(searchObj)
          //   .then(data => {
          //     self.loading = false;
          //     self.dataTrue = false;
          //     //   console.log("列表", JSON.parse(JSON.stringify(data)));
