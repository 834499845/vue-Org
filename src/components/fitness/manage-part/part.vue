                        ...
                        <li class="dataLi" v-show="lists.length > 0" v-for="(item,index) in lists">
                            <div class="dataCheck">
                                <el-checkbox-group v-model="checkList" @change="handleCheckedCitiesChange">
                                    <el-checkbox :label="item" :key="item.siteName">{{checkNull}}</el-checkbox>
                                </el-checkbox-group>
                                <div>
                                    {{index + 1}}
                                </div>
...
export default {
  ...
  methods: {
    ...
    updateClick(index) {
      ...
      this.addShow = true;
      this.name = this.lists[index].siteName;
      this.upOrAd = 2;
      this.upIndex = index;
    },
    // addtion
      ...
      if (self.butAddClickTrue == false) {
        if (self.isattributeValue == true) {
          if (self.upOrAd == 1) {
            ...
            self.butAddClickTrue = true;
            self
              .getfitnessSiteControllerupdate({
    ...
    delClick() {
      ...
      if (self.delClickTrue == false) {
        ...
        self
          .getfitnessSiteControllerdeleteById({
            ids: idStr
          })
          .then(data => {
            self.deling = false;
